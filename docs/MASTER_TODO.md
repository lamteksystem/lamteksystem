# Lamtek â€“ Master TODO (authoritative)
...
| A2 | **DONE** | `src/lib/deliveryWindows.ts` â€“ load windows + rules, label helper, MVP slot validation (weekday + same-day cut-off in Europe/London). |
| A3 | **DONE** | Admin **Delivery windows** at `/admin/delivery-windows` (CRUD windows + weekday cut-off / lead rules). |

### Wave B â€“ Customer checkout & fulfilment
| ID | Status | Deliverable |
|----|--------|-------------|
...
### Wave D â€“ Amendments & extras (enforcement)
| ID | Status | Deliverable |
|----|--------|-------------|
...
| D3 | **DONE** | **Create extras order** header action â†’ `/admin/create-order?customer=&parentOrder=&linkReason=extras` with parent delivery copy + linkage on insert. |

### Wave E â€“ Ledger & accounting
| ID | Status | Deliverable |
|----|--------|-------------|
...
### Wave H â€“ Product discovery & guided ordering
| ID | Status | Deliverable |
|----|--------|-------------|
...
| H3 | **TODO** | Compare basket (2â€“4 SKU). |
...
### Wave K â€“ Integrations
| ID | Status | Deliverable |
|----|--------|-------------|
| K1 | **TODO** | Accounting CSV export. |
| K2 | **TODO** | CSV â†’ basket import. |
| K3 | **TODO** | â€œSend us your quoteâ€ intake. |
...
## Phase 0 â€“ Baseline audit & acceptance criteria
- **IN PROGRESS**: Map flows and Definition of Done per area (see `docs/acceptance/`, `docs/qa/`).

## Historical backlog (docs-only â€œnext 10â€) â€“ archived reference
Items 1â€“10 (guided ordering docs, click & collect **design**, delivery windows **design**, amendments **policy**, exports **spec**, ledger **policy**, admin matrix) are **DONE** as documentation. Implementation is tracked in **Execution waves** above.

## Phase 1 â€“ Customer ordering overhaul
- **IN PROGRESS**: Guided wizard + baskets (see Wave H).
- **IN PROGRESS**: Quote exports (Wave C).

## Phase 2 â€“ Checkout, delivery, Click & Collect
- **MVP**: Click & collect (shipped in app + DB); delivery windows (Wave B).
- **POST-MVP**: Delivery pricing, partial fulfilment, POD.

## Phase 3 â€“ Returns / refunds / accounting
- See Wave E + MASTER bullets in prior versions (`returns`, `credit limits`, allocations, exports).

## Phase 4 â€“ Warehouse / ops
- See Wave J.
...
*Mobile / remote editing: open this repository in the Cursor mobile app from the same account or remote host (GitHub, SSH, or synced folder). The agent cannot remotely unlock or pair your phoneâ€”use Cursorâ€™s documented mobile workflow.*
