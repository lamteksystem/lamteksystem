-- Parent category "Drawer Fronts" + ensure Accessories children have correct parent_id.

do $$
declare
  v_accessories_id uuid;
  v_drawer_fronts_id uuid;
begin
  select id into v_accessories_id from public.categories where lower(btrim(name)) = 'accessories' and parent_id is null limit 1;
  ...
  from (values ('Cutlery Trays', 'cutlery-trays', 1), ('Lighting', 'lighting', 2), ('Misc', 'misc', 3)) as v(name, slug, sort_order)
  where not exists (
    select 1 from public.categories c
    where lower(btrim(c.name)) = lower(btrim(v.name)) and c.parent_id = v_accessories_id
  );

  update public.categories c
  set parent_id = v_accessories_id
  where lower(btrim(c.name)) in ('cutlery trays', 'lighting', 'misc')
    and (c.parent_id is null or c.parent_id <> v_accessories_id);

  select id into v_drawer_fronts_id from public.categories where lower(btrim(name)) = 'drawer fronts' and parent_id is null limit 1;
  if v_drawer_fronts_id is null then
    insert into public.categories (name, slug, sort_order, category_kind)
    values ('Drawer Fronts', 'drawer-fronts', 46, 'product_type');
  end if;
end;
$$;

-- Extend prune keep-list (matches coreCatalogueCategories.ts).
...
begin
  ...
  update public.products p set category_id = null from doomed d where p.category_id = d.id;
