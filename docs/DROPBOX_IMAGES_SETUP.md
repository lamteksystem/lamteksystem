...
The zip **product-images-download.zip** is already in your project (downloaded from that link). If itâ€™s still locked, close the browser/download manager, then:
...
1. **Extract the zip**
   - In File Explorer go to `c:\Users\info\Desktop\Lamtek`.
   - Rightâ€‘click **product-images-download.zip** â†’ **Extract All**.
...
2. Extract the downloaded zip to `c:\Users\info\Desktop\Lamtek\product-images` (or any folder).
3. Run:
   ```bash
   npm run assign-images -- "product-images"
   ```
