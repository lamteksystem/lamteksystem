...
|--------|------------|--------------------------------------|
| **GitHub** | `git` remote = `https://github.com/lamteksystem/lamteksystem`, branch `main`, initial commit locally | The machine is logged in as `trademouldingsltd`, which has **read-only** on that repo. **Push** needs a user with `push` (owner or invited collaborator with write). |
| **Supabase (API)** | The app can talk to `https://jhmepthfxnpmwpjobumj.supabase.co` with your publishable key. | The database has **no `public.products` table yet** — run migrations (below). The Supabase **CLI** was unlinked from the old TradeMouldings project; linking or `db push` to Lamtek needs your **Lamtek Supabase account** or a **Postgres `DATABASE_URL`**. |

---

## 1) GitHub — one-time (pick one)

### A. Invite the current computer’s Git user (quickest for this machine)
...
3. **Add people** → invite user **`trademouldingsltd`** with **Write** role.
4. Accept the invite (from the trademouldingsltd account).
...
1. Create a **classic** PAT (lamteksystem) with `repo` scope: GitHub → **Settings** → **Developer settings** → **PATs**.
2. Run (replace `TOKEN`):
...
## 2) Supabase — migrations on project `jhmepthfxnpmwpjobumj`
...
### Option A — CLI: login as Lamtek, then link and push (recommended for ongoing work)
...
The old link to the Trade Mouldings project has been **unlinked** (`supabase unlink`).

### Option B — `DATABASE_URL` (no `supabase link`)

1. In the Supabase dashboard: **Project Settings** → **Database** → **Connection string** → **URI** and replace `[YOUR-PASSWORD]` with the **database** password.
...
If the password has special characters, you may need to **percent-encode** them in the URL for the CLI (Supabase’s copy button usually gives a working string).
...
Never put `SERVICE_ROLE` or `DATABASE_URL` in the frontend host — server/scripts only.
